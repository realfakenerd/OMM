...
6. **Rust Static Analysis (Backend Tasks):**
   - For any changes involving Rust code, run the following checks:
     ```bash
     cargo check
     cargo clippy -- -D warnings
     ```
   - **CRITICAL:** These checks must pass with no errors or warnings before proceeding.
...
- [ ] Rust code passes `cargo check` and `cargo clippy` with no warnings
- [ ] All public functions/methods are documented (e.g., docstrings, JSDoc, GoDoc, Rust docs)
- [ ] Type safety is enforced (e.g., type hints, TypeScript types, Rust types)
...
npm install
# Setup Tauri Android target (ensure Android SDK/NDK are installed)
...
### Before Committing
```bash
# Full check: lint, type-check, and test
npm run check && cargo check && cargo clippy -- -D warnings && cargo test && npm test
```
